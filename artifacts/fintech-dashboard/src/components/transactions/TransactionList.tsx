...
export function TransactionList({ transactions, onDelete, onEdit, deletingId }: TransactionListProps) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700/60 dark:bg-slate-950 dark:shadow-none">
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-slate-900 dark:text-slate-50 truncate max-w-[150px]">{tx.description}</p>
                      <p className="text-[11px] text-slate-500">{formatDate(tx.date)}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className={cn("text-sm font-bold", tx.type === "income" ? "text-emerald-500" : "text-rose-500")}>
                ...
                <div className="flex items-center justify-between pt-1">
                  <span className="text-[11px] text-slate-500 font-medium px-2 py-1 rounded-lg bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                    {tx.category}
                  </span>
                  <div className="flex items-center gap-1">
                    <button
                      onClick={() => onEdit(tx)}
                      className="p-2 text-slate-500 active:bg-blue-500/10 active:text-blue-500 transition-colors rounded-lg"
                    >
                    ...
                    <button
                      ...
                      className="p-2 text-slate-500 active:bg-rose-500/10 active:text-rose-500 transition-colors rounded-lg disabled:opacity-40"
                    >
      ...
      {/* Desktop view (table) */}
      <div className="hidden md:block border-t border-slate-200 dark:border-slate-700/60 overflow-x-auto">
